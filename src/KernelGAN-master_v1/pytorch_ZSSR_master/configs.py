...
class Config:
    ...
    scale_factors = [
        [2.0, 2.0]
    ]  # list of pairs (vertical, horizontal) for gradual increments in resolution
    base_change_sfs = (
        []
    )  # list of scales after which the input is changed to be the output (recommended for high sfs)
    max_iters = 3000
    min_iters = 256
    ...
    downscale_method = (
        "cubic"  # a string ('cubic', 'linear'...), has no meaning if kernel given
    )
    ...
    learn_residual = (
        True  # when true, we only learn the residual from base interpolation
    )
    init_variance = 0.1  # variance of weight initializations, typically smaller when residual learning is on
    back_projection_iters = [
        2
    ]  # for each scale num of bp iterations (same length as scale_factors) [10]  # for each scale num of bp iterations (same length as scale_factors)
    random_crop = True
    crop_size = 128
    ...
    learning_rate = 0.001
    learning_rate_change_ratio = (
        1.5  # ratio between STD and slope of linear fit, under which lr is reduced
    )
    ...
    augment_allow_rotation = (
        False  # changed from True  # recommended false for non-symmetric kernels
    )
...
X2_GRADUAL_IDEAL_CONF.scale_factors = [
    [1.0, 1.5],
    [1.5, 1.0],
    [1.5, 1.5],
    [1.5, 2.0],
...
#     [2.0, 2.0],
# ]
# X2_REAL_CONF.back_projection_iters = [0,0,0,0,0,0] #[6, 6, 8, 10, 10, 12]
# X2_REAL_CONF.back_projection_iters = [X2_REAL_CONF.back_projection_iters[0], X2_REAL_CONF.back_projection_iters[0]]