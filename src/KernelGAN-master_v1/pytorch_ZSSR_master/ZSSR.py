...
class ZSSR:
    ...
    def __init__(self, input_img, conf=Config(), ground_truth=None, kernels=None):
        ...
        self.input = self.preprocessing_image(input_img)
        # if self.input[2] == 4:
        self.Y = False
        if len(self.input) == 2:
            self.Y = True
        ...
        self.gt = (
            ground_truth if type(ground_truth) is not str else img.imread(ground_truth)
        )
        # gt is ndarray
        # Preprocess the kernels. (see function to see what in includes).
        self.kernels = kernels #preprocess_kernels(kernels, conf)
    ...
    def preprocessing_image(self, input_img_path):
        input_image = img.imread(input_img_path)
        if input_image.shape[-1] == 4:
            input_image = input_image[:, :, :3]
        elif len(input_image.shape) == 2:
            input_image = np.expand_dims(input_image, axis=-1)
        return input_image / 255.0 if input_image.dtype == 'uint8' else input_image
    ...
    def run(self):
        ...
        for self.sf_ind, (sf, self.kernel) in enumerate(
            zip(self.conf.scale_factors, self.kernels *len(self.conf.scale_factors))
        ):
            # verbose
            ...
            if self.conf.save_results:
                sf_str = "".join(
                    "X%.2f" % s for s in self.conf.scale_factors[self.sf_ind]
                )
                plt.imsave(
                    "%s/%s_zssr_%s.png"
                    % (
    ...
    def init_parameters(self):
        ...
        self.gt_per_sf = (
            ...
            if (
                self.gt is not None
                and self.sf is not None
                and np.any(np.abs(self.sf - self.conf.scale_factors[-1]) > 0.01)
            )
            else self.gt
        )
    ...
    def forward_backward_pass(self, lr_son, hr_father, criterion, optimizer):
        ...
        interpolated_lr_son = imresize(
            lr_son, self.sf, hr_father.shape, self.conf.upscale_method
        )
        if self.Y == True:
    ...
    def forward_pass(self, lr_son, hr_father_shape=None):
        # First gate for the lr-son into the network is interpolation to the size of the father
        interpolated_lr_son = imresize(
            lr_son, self.sf, hr_father_shape, self.conf.upscale_method
        )
        if self.Y == True:
            interpolated_lr_son = (
            ...
            interpolated_lr_son = (
                torch.Tensor(interpolated_lr_son).permute(2, 0, 1)
            ).unsqueeze_(0)
        if self.cuda:
        ...
        return np.clip(
            np.squeeze(
                self.model(interpolated_lr_son)
                .cpu()
                .detach()
                .permute(0, 2, 3, 1)
                .numpy()
    ...
    def learning_rate_policy(self):
        ...
        if (
            not (1 + self.iter) % self.conf.learning_rate_policy_check_every
            and self.iter - self.learning_rate_change_iter_nums[-1]
            > self.conf.min_iters
        ):
            ...
            [slope, _], [[var, _], _] = np.polyfit(
                self.mse_steps[
                    -(self.conf.learning_rate_slope_range // self.conf.run_test_every) :
                ],
                self.mse_rec[
                    -(self.conf.learning_rate_slope_range // self.conf.run_test_every) :
                ],
    ...
    def quick_test(self):
        ...
        self.mse = (
            self.mse
            + [np.mean(np.ndarray.flatten(np.square(self.gt_per_sf - self.sr)))]
        ...
        self.reconstruct_output = self.forward_pass(
            self.father_to_son(self.input), self.input.shape
        )
        self.mse_rec.append(
            np.mean(np.ndarray.flatten(np.square(self.input - self.reconstruct_output)))
        )

        # 3. True MSE of simple interpolation for reference (only if ground-truth was given)
        interp_sr = imresize(
            self.input, self.sf, self.output_shape, self.conf.upscale_method
        )
        self.interp_mse = (
            self.interp_mse
            + [np.mean(np.ndarray.flatten(np.square(self.gt_per_sf - interp_sr)))]
            if self.gt_per_sf is not None
            else None
        )
        ...
        interp_rec = imresize(
            self.father_to_son(self.input),
            self.sf,
            self.input.shape[0:2],
            self.conf.upscale_method,
        )
        self.interp_rec_mse.append(
            np.mean(np.ndarray.flatten(np.square(self.input - interp_rec)))
        )
    ...
    def train(self):
        ...
        for self.iter in range(self.conf.max_iters):
            ...
            self.train_output = self.forward_backward_pass(
                self.lr_son, self.hr_father, criterion, optimizer
            )
    ...
    def father_to_son(self, hr_father):
        ...
        return np.clip(
            lr_son + np.random.randn(*lr_son.shape) * self.conf.noise_std, 0, 1
        )
    ...
    def final_test(self):
        ...
        for k in range(
            0, 1 + 7 * self.conf.output_flip, 1 + int(self.sf[0] != self.sf[1])
        ):
            # Rotate 90*k degrees and mirror flip when k>=4
            test_input = (
                np.rot90(self.input, k) if k < 4 else np.fliplr(np.rot90(self.input, k))
            )
            ...
            tmp_output = (
                np.rot90(tmp_output, -k)
                if k < 4
                else np.rot90(np.fliplr(tmp_output), -k)
            )
            ...
            for bp_iter in range(self.conf.back_projection_iters[self.sf_ind]):
                tmp_output = back_projection(
                    tmp_output,
                    self.input,
                    down_kernel=self.kernel,
        ...
        for bp_iter in range(self.conf.back_projection_iters[self.sf_ind]):
            almost_final_sr = back_projection(
                almost_final_sr,
                self.input,
                down_kernel=self.kernel,
                up_kernel=self.conf.upscale_method,
    ...
    def base_change(self):
        ...
        if (
            abs(
                self.conf.scale_factors[self.sf_ind]
                - self.conf.base_change_sfs[self.base_ind]
            )
            < 0.001
        ):
    ...
    def plot(self):
        plots_data, labels = zip(
            *[
                (np.array(x), l)
                for (x, l) in zip(
                    [self.mse, self.mse_rec, self.interp_mse, self.interp_rec_mse],
                    [
                        "True MSE",
                        "Reconstruct MSE",
                        "Bicubic to ground truth MSE",
                        "Bicubic to reconstruct MSE",
                    ],
                )
                if x is not None
            ]
