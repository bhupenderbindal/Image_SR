...
class SumOfWeightsLoss(nn.Module):
    ...
    def forward(self, kernel):
        # [1], []
        # unsqueeze added to match dimension as ones return a 1-dimensional
...
class CentralizedLoss(nn.Module):
    ...
    def forward(self, kernel):
        ...
        # [2,1], [2]
        # added unsqueeze(1) to match the dimension
        return self.loss(torch.stack((torch.matmul(r_sum, self.indices) / torch.sum(kernel),
                                      torch.matmul(c_sum, self.indices) / torch.sum(kernel))), self.center.unsqueeze(1))
