# Adapted from: https://github.com/whiplashoo/split-image/blob/23d445bb94759ff2da49c6828945a794dabe6fef/src/split_image/split.py
...
def split_image(
    ...
    if should_square:
        ...
        im_r.save(outp_path, format="JPEG", subsampling=0, quality=100)
        im = im_r
    ...
    for i in range(0, rows):
        for j in range(0, cols):
            ...
            # update to avoid compression
            outp.save(outp_path, format="JPEG", subsampling=0, quality=100)
            n += 1
    if should_cleanup:
        if not should_quiet:
...
def reverse_split(
    paths_to_merge, rows, cols, image_path, should_cleanup, should_quiet=False
):
    ...
    new_image.save(image_path, format="JPEG", subsampling=0, quality=100)
    if should_cleanup:
...
@dataclass
class DefaultSplittingArgs:

    # SETTING REMAINING ARGUMENTS
    square: bool = False
    reverse: bool = False
    cleanup: bool = False
    load_large_images: bool = True
    quiet: bool = False


def image_splitter(args):
    ...
    if os.path.isdir(image_path):
        ...
        if args.reverse:
            ...
            breakpoint()
            print("--------------")
            a = [f for f in os.listdir(os.getcwd()) if re.match(expr, f)]
            print(a)
            paths_to_merge = sorted(
                [f for f in os.listdir(os.getcwd()) if re.match(expr, f)],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--quiet", action="store_true", help="Run without printing any messages."
    )
