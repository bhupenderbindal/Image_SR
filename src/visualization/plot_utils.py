...
def calc_metrics(img1, img2):
    ...
    ssim = ski.metrics.structural_similarity(img1, img2, data_range=255.0, channel_axis=-1, gaussian_weights=False, full=False)
...
def inference_plot_and_save(gt_img, input_lr, output_hr, output_dir =None, output_name=None):
    ...
    # print("======= saving bicubic and  output HR of model ========")
    ...
    psnr_bicubic, ssim_bicubic, lpips_distance_bicubic = calc_metrics(gt_img, bicubic_hr) 
    ...
    ax[0].imshow(gt_img)
    ax[0].set_title("GT")
    ax[0].set_xlabel("PSNR = " +str(psnr_gt) + "  SSIM = " +str(ssim_gt)+ "  LPIPS = " +str(lpips_distance_gt))
    ax[1].imshow(bicubic_hr)
    ax[1].set_xlabel("PSNR = " +str(psnr_bicubic)+ "  SSIM = " +str(ssim_bicubic)+ "  LPIPS = " +str(lpips_distance_bicubic))
    ax[1].set_title("bicubic HR") 
    ax[2].imshow(output_hr)
    ax[2].set_title("HR output")
    ax[2].set_xlabel("PSNR = " +str(psnr_output)+ "  SSIM = " +str(ssim_output)+ "  LPIPS = " +str(lpips_distance_output))
    
    # if save:
    #     plt.imsave((output_dir + img[1] + ".png"), img[0])

    if output_dir and output_name:
        plt.savefig((output_dir +output_name +"compare" + datetime.now().strftime('_%b_%d_%H_%M_%S_%f')  + ".png"),bbox_inches='tight')
    # plt.savefig("xx.svg")
    # plt.show()
...
def compare_images(img1, img2):
    ...
    psnr, ssim = calc_metrics(img1, img2) 
        
    ax[0].imshow(img1)
    ax[0].set_title("GT")
    ax[1].imshow(img2)
    ax[1].set_xlabel("PSNR = " +str(psnr)+ "  SSIM = " +str(ssim))
    ax[1].set_title("img2") 
    plt.show()
