...
def compute_l1_distance(x: torch.Tensor, y: torch.Tensor):
    # Ensure that both x and y are on the same device
    device = x.device

    # Calculate L1 distance between x and y
    with torch.no_grad():
        # Move x and y to CPU to avoid potential OOM errors
        x_cpu = x.cpu()
        y_cpu = y.cpu()

        # Calculate L1 distance between x and y
        N, C, H, W = x_cpu.size()
        x_vec = x_cpu.view(N, C, -1)
        y_vec = y_cpu.view(N, C, -1)
    ...
    # Move dist back to the original device
    dist = dist.to(device)
...
def compute_l2_distance(x, y):
    ...
    A = y_vec.transpose(1, 2) @ x_vec
    dist = y_s.unsqueeze(2) - 2 * A + x_s.unsqueeze(1)
    dist = dist.clamp(min=0.)
...
def create(shape):
    # Get the shape of the target features
    N, C, H, W = shape #T_features.size()

    # Create spatial grid
    rows = torch.arange(H, dtype=torch.float32) / (H - 1) * 255.0
    cols = torch.arange(W, dtype=torch.float32) / (W - 1) * 255.0
    grid_y, grid_x = torch.meshgrid(rows, cols)
    features_grid = torch.stack((grid_x, grid_y), dim=0).unsqueeze(0).repeat(N, 1, 1, 1)

    # Compute spatial flow using L2 distance
    # cs_flow_sp = create_using_L2(features_grid, features_grid, nnsigma, b)
